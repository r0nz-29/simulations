...
export const renderScene = () => {
  ...
  dragControls.addEventListener("drag", (e) => {
    ...
    dl.add(newRvector);
    if (configs.showAngle) {
      const rightAngleG = new THREE.EdgesGeometry(
        new THREE.BoxGeometry(0.2, 0.125, 0)
      );
      ...
      rightAngle.position.copy(
        new THREE.Vector3(
          point.position.x + 0.125,
          point.position.y,
          point.position.z
        )
      );
      rightAngle.rotation.x = Math.PI / 2;
      rightAngle.rotation.z = Math.atan2(r, point.position.z);
      rightAngle.name = "rightAngle";
      newRvector.add(rightAngle);
    }
  ...
  pane.addInput(configs, "showAngle").on("change", (e) => {
    // e.value ? rVector.add(rightAngle) : rVector.remove(rightAngle);
    configs.showAngle = e.value;
    if (e.value) {
      const rightAngleG = new THREE.EdgesGeometry(
        new THREE.BoxGeometry(0.2, 0.125, 0)
      );
  ...
  const animate = function () {
    ...
    if (running) {
      ...
      rightAngle.rotation.x = Math.PI / 2;
      rightAngle.rotation.z = Math.atan2(r, z);
      rightAngle.name = "rightAngle";
      rVector.add(rightAngle);
      const direction = new THREE.Vector3().crossVectors(rvector, dlVector);
      const newdb = utils.createArrow(
